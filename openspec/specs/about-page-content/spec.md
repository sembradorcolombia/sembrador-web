...
The `/acerca` page's narrative content — who the church is, its vision and mission, and the values and beliefs it holds — lives in a dedicated `aboutPage` singleton rather than being borrowed from site-wide settings, so editors can shape the page without touching global configuration.
## Requirements

...
The admin dashboard (`/dashboard`) hosts more than one unrelated dataset, so its content is divided into named sections selected by an in-page tab switcher. Each section owns its own data fetching and its own loading and error states, while the admin guard and the page shell apply across all of them.
## Requirements

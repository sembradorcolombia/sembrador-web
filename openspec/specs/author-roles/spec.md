...
An author can be a preacher, a leader, an editor, or several at once. A multi-valued `roles` field replaces the earlier single `role` string, and leadership-specific fields appear only for authors who actually lead, keeping the rest of the author form uncluttered.
## Requirements

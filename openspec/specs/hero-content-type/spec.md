...
Page banners are content, not layout. A `hero` document keyed to each top-level page lets editors change any banner's heading, image, lead text, and call to action without a code change, and a single shared component renders all of them so no page carries its own banner markup.
## Requirements

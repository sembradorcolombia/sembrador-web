...
How `/dashboard` and `/login` decide whether a visitor is an admin: the claim is verified against the authentication server rather than read from client-controlled storage, every failure denies access, and rejected sessions are cleared where they are found. Data access does not depend on any of it — database policies remain the boundary.
## Requirements

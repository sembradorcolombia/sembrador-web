...
- [x] 1.5 Create `schemaTypes/aboutPage.ts`: `description` (Portable Text array), `vision` (text), `mission` (text), `coreValues[]` and `coreBeliefs[]` (arrays of `{ title (required), description }` where description is a constrained Portable Text array — lists, emphasis and links, no headings), each with a preview
...
- [x] 1.7 Change `schemaTypes/author.ts`: replace `role` (string) with `roles` (array of string, `options.list` of speaker/leader/publisher, `options.layout: "grid"`, `rule.unique()`) — keep the old `role` field in place for now so migration can read it
...
- [x] 1.13 Run the Studio locally and confirm every validation rule fires as intended (required fields, partial CTA, duplicate key, conditional leadership title)
...
- [x] 2.5 Populate `vision`, `mission`, `coreValues`, and `coreBeliefs` with the content the church wants to publish (needs source copy from the content owner)
- [x] 2.6 Upload the confession of faith and any other PDFs into `aboutPage.documents`
- [x] 2.7 Assign `roles[]` on every existing author from their stored `role` value; report any value that does not map to a defined option for manual assignment rather than dropping it
- [x] 2.8 Set `leadershipTitle` and `leadershipOrder` for each author carrying the `leader` role
- [x] 2.9 Verify in the Studio that no author lost their designation and that the leadership set is who it should be
- [x] 2.10 Create the `blog`, `eventos`, `conectar`, and `dar` heroes; until each exists the page keeps rendering its previous heading and subtitle as the fallback
...
- [x] 5.5 Create the leadership section: image with alt text falling back to the leader's name, name, and leadership title, centered; the bio is not shown on this page; omit the section when there are no leaders
...
- [x] 7.5 Test the leadership section: only `leader`-role authors appear; ordering with and without `leadershipOrder`; the bio is never rendered; section omitted when there are no leaders; image alt falls back to the name

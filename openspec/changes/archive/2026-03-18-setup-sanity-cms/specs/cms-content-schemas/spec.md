...
The Sanity Studio SHALL define a `siteSettings` singleton document type for global site configuration.
...
- **AND** the schema SHALL accept optional fields: `heroImage` (image), `aboutDescription` (text), `aboutLocation` (string), `aboutServiceTimes` (string), `socialLinks` (array of objects with `platform` string and `url` URL)
